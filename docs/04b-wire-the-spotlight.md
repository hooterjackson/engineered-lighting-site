...
3. **Inputs.** Each IN stub lands in its own breadboard row. Into each row: a **10 kΩ to the ground rail**, and a jumper to its GPIO — **10 → IN1, 11 → IN2, 18 → IN3**. Signals only on the breadboard, same law as always. (The soldered, fixture-lane placement of these same three resistors is drawn in [Doc 8's small-parts maps](08-build-the-fixture.md#the-three-small-parts-and-where-every-one-goes).)
