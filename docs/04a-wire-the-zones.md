...
This is the BoM's PicoBuck-class driver (this unit's chips read AL8860). **Correction to an earlier version of this sentence:** it said ~330 mA per channel, read off a single R300 sense resistor — the board carries **two** R300s per channel, and the parallel pair puts each channel at **~660 mA** ([Doc 4b has the math](04b-wire-the-spotlight.md#the-driver-demystified)). It forces a fixed **current** through three **separate** outputs — the tape is a fixed-**voltage**, common-anode part with one shared 24 V, so there is nothing here for it to connect to. Set it aside until spotlight day — that day's chapter is **[Doc 4b](04b-wire-the-spotlight.md)**. The rule runs both ways: the spotlight's bare LED star never touches the 24 V bus either.
